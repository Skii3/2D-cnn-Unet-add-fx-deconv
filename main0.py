...
eng.addpath(eng.genpath('../00-compare_residual_and_without_residual/SegyMAT'))
eng.addpath(eng.genpath('../00-compare_residual_and_without_residual/SeismicLab'))
...
def train():
    ...
    nn1 = configPara.nn1
    input1 = tf.placeholder('float32', [1, nn1, nn1, 2], name='input_image_forward1')
    ...
    temp_snr2 = tf.reduce_sum(tf.square(tf.abs(target1))) / \
               tf.reduce_sum(tf.square(tf.abs(target1 - tf.expand_dims(tf.expand_dims(input1[0,:,:,0],axis=0),axis=3))))
    snr_input = 10.0 * tf.log(temp_snr2) / tf.log(10.0)

    temp_snr3 = tf.reduce_sum(tf.square(tf.abs(target1))) / \
                tf.reduce_sum(tf.square(tf.abs(target1 - tf.expand_dims(tf.expand_dims(input1[0,:,:,1],axis=0),axis=3))))
    snr_input_fxdeconv = 10.0 * tf.log(temp_snr3) / tf.log(10.0)
    ...
    for epoch in range(0, n_epoch + 1):
        ...
        snr_input_min = 0
        snr_output_min = 0
        snr_input_fxdeconv_max = 0
        snr_input_fxdeconv_min = 0
        for idx in range(0, 1000, 1):
            ...
            while (1):
                ...
                if np.size(imgs_input1) != 1:
                    break
            # fx-deconv 处理
            imgs_input1_fxdeconv = np.float64(eng.filter_fx(matlab.double(imgs_input1.squeeze().tolist()), nargout=1))
            imgs_input1_all = np.concatenate([imgs_input1,np.expand_dims(np.expand_dims(imgs_input1_fxdeconv,axis=0),axis=3)],axis=3)
            tvDiff_loss, L1_loss, _, snr_input1, snr_output1,snr_input_fxdeconv1 \
                = sess.run([tvDiff_loss_forward, L1_loss_forward, optim_forward, snr_input, snr_output, snr_input_fxdeconv], \
                           {input1: imgs_input1_all, target1: imgs_target1})

            if idx == 0:
                snr_input_max = snr_input1
                snr_input_min = snr_input1
                snr_output_max = snr_output1
                snr_input_fxdeconv_max = snr_input_fxdeconv1
                snr_output_min = snr_output1
                snr_input_fxdeconv_min = snr_input_fxdeconv1
            else:
        ...
        print("[*] Epoch [%2d/%2d] %4d time: %4.4fs, all_loss: %.8f, tvDiff_loss: %.8f, L1_loss: %.8f, "
              "max_snr: %.4f, %.4f => %.4f, min_snr: %.4f, %.4f => %.4f" \
              % (epoch, n_epoch, n_iter, time.time() - epoch_time, sum_all_loss / n_iter, sum_tvDiff_loss / n_iter,
                 sum_L1_loss / n_iter,snr_input_max,snr_input_fxdeconv_max,\
                 snr_output_max,snr_input_min,snr_input_fxdeconv_min,snr_output_min))
        ...
        if (epoch % configPara.save_model_freq == 0):
            ...
            while (1):
                ...
                if np.size(imgs_input) != 1:
                    break
            # fx-deconv 处理
            imgs_input_fxdeconv = np.float64(eng.filter_fx(matlab.double(imgs_input.squeeze().tolist()), nargout=1))
            imgs_input_all = np.concatenate(
                [imgs_input, np.expand_dims(np.expand_dims(imgs_input_fxdeconv, axis=0), axis=3)], axis=3)

            out = sess.run(forward1, {input1: imgs_input_all})
            result = np.concatenate((imgs_input.squeeze(), imgs_input_fxdeconv, out.squeeze(),  imgs_target.squeeze()), axis=1)
            ...
            for i in range(9):
                ...
                imgs_input_all = np.concatenate(
                    [imgs_input, np.expand_dims(np.expand_dims(imgs_input_fxdeconv, axis=0), axis=3)], axis=3)
                out = sess.run(forward1, {input1: imgs_input_all})
                out = np.concatenate((imgs_input.squeeze(), imgs_input_fxdeconv, out.squeeze(), imgs_target.squeeze()), axis=1)
...
def runTest(test_imgs, target_imgs, sess):
    ...
    input_image_forward_large = tf.placeholder('float32', [1, w2, h2, 2], name='input_test')
    image_forward_large = generator(input_image_forward_large, reuse=True, net_name="net_forward")
    input_snr_sum = 0
    output_snr_sum = 0
    input_snr_fxdeconv_sum = 0
    N = 5
    for i in range(0, N, 1):
        ...
        # fx-deconv 处理
        imgs_input_fxdeconv = np.float64(eng.filter_fx(matlab.double(inputImage2.squeeze().tolist()), nargout=1))
        imgs_input_all = np.concatenate(
            [inputImage2, np.expand_dims(np.expand_dims(imgs_input_fxdeconv, axis=0), axis=3)], axis=3)


        out = sess.run(image_forward_large, {input_image_forward_large: imgs_input_all})
        ...
        tmp_snr0 = np.sum(np.square(np.abs(targetImage2.squeeze()))) / np.sum(
            np.square(np.abs(targetImage2.squeeze() - imgs_input_fxdeconv)))
        input_snr_fxdeconv = 10.0 * np.log(tmp_snr0) / np.log(10.0)  # 输入图片的snr

        input_snr_sum = input_snr_sum + input_snr
        output_snr_sum = output_snr_sum + output_snr
        input_snr_fxdeconv_sum = input_snr_fxdeconv_sum + input_snr_fxdeconv
        ...
        np.savetxt(configPara.test_save_dir + '/%d_noise_fxdeconv.txt' % i, imgs_input_fxdeconv,
                   delimiter=' ')
        ...
        scipy.misc.imsave(configPara.test_save_dir + '/%d_noise_fxdeconv.png' % i,
                          reScale(imgs_input_fxdeconv))
    input_snr_sum = input_snr_sum / N
    output_snr_sum = output_snr_sum / N
    input_snr_fxdeconv_sum = input_snr_fxdeconv_sum / N
    print('[*]input snr average: %.2f, input snr fxdeconv average: %.2f, output snr average: %.2f' % (input_snr_sum, input_snr_fxdeconv_sum, output_snr_sum))
...
def runTest_test(test_imgs, target_imgs,sess):
    for i in range(0, len(test_imgs), 1):
        ...
        print('[*]input snr: %.2f, input fxdeconv snr: %.2f, output snr: %.2f' % (input_snr,input_snr_fxdeconv, output_snr))
        ...
        np.savetxt(configPara.test_save_dir + '/%d_noise_fxdeconv.txt' % round(input_snr), imgs_input_fxdeconv, delimiter=' ')
        # np.savetxt(configPara.test_save_dir+'/target_%d.txt' % i, test_img, delimiter=' ')
        # out = scipy.misc.imresize(reScale(out.squeeze()), size=[w, h], interp='bicubic', mode=None)
        # out=np.concatenate((inputImage, out),axis=1)
        ...
        scipy.misc.imsave(configPara.test_save_dir + '/%d_noise_fxdeconv.png' % round(input_snr), reScale(imgs_input_fxdeconv))
...
def test_20():
    ...
    for file in file_all:
        ...
        print('[*]input snr: %.2f, input fxdeconv snr: %.2f, output snr: %.2f' % (
        input_snr/20, input_snr_fxdeconv/20, output_snr/20))
...
def test_plut_15():
    ...
    for i in range(-5,16,1):
        ...
        print('[*]input snr: %.2f, input fxdeconv snr: %.2f, output snr: %.2f' % (
            input_snr/10, input_snr_fxdeconv/10, output_snr/10))
